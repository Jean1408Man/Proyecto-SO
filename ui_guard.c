...
static void aplicar_configuracion(GtkButton *btn, gpointer user_data)
{
    ...
    for (int i = 0; i < total; ++i) {
        /* Índice 2 corresponde a "Escaneo de Puertos" según el arreglo servicios[] */
        if (i == 2) {
...
main(int argc, char *argv[])
{
    ...
    gtk_box_pack_start(GTK_BOX(config_box), start_port_label, FALSE, FALSE, 10);
    gtk_box_pack_start(GTK_BOX(config_box), start_port_spin,  FALSE, FALSE, 0);

    gtk_box_pack_start(GTK_BOX(config_box), end_port_label,   FALSE, FALSE, 10);
    gtk_box_pack_start(GTK_BOX(config_box), end_port_spin,    FALSE, FALSE, 0);

    gtk_box_pack_start(GTK_BOX(config_box), btn_aplicar,       FALSE, FALSE, 20);
